...
impl ColoredCfg {
    ...
    pub fn paired_label(&self, cfg_label: CfgLabel) -> PairedLabel {
        match self.clone2origin.get(&cfg_label) {
            Some(origin) => PairedLabel(*origin, cfg_label),
            None => PairedLabel(cfg_label, cfg_label),
        }
    }
    ...
    pub fn as_labeled_cfg(&self) -> LabeledCfg {
        ...
        LabeledCfg::from_edges(self.paired_label(self.cfg.entry), &edges).unwrap()
    }
}
...
mod reducing_tests {
    ...
    pub fn test_as_labeled_cfg() {
        ...
        let mut cgraph = ColoredCfg::new(&graph);
        cgraph.reduce_colors();
        let reduced = cgraph.as_labeled_cfg();
